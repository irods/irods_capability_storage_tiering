...
class TestStorageTieringPlugin(ResourceBase, unittest.TestCase):
    def setUp(self):
        ...
        with session.make_session_for_existing_admin() as admin_session:
            ...
            admin_session.assert_icommand('''imeta set -R rnd1 irods::storage_tiering::query "SELECT DATA_NAME, COLL_NAME, USER_NAME, USER_ZONE, DATA_REPL_NUM where RESC_NAME = 'ufs2' || = 'ufs3' and DATA_ACCESS_TIME < 'TIME_CHECK_STRING' and DATA_MODIFY_TIME < 'TIME_CHECK_STRING'"''')
            ...
            with session.make_session_for_existing_admin() as admin_session:
                with session.make_session_for_existing_user('alice', 'apass', lib.get_hostname(), zone_name) as alice_session:
                    ...
                    try:
                        ...
                        # sleep a little less than irods::storage_tiering::time
                        time.sleep(2)
                        # touch the file again, updating the mtime only
                        alice_session.assert_icommand('itouch ' + filename)

                        # test updated mtime prevents staging to tier 1
                        time.sleep(3)
                        invoke_storage_tiering_rule()
                        wait_for_empty_queue(
                            lambda: alice_session.assert_icommand('ils -L ' + filename, 'STDOUT_SINGLELINE', 'rnd0')
                        )
...
class TestStorageTieringPluginMultiGroup(ResourceBase, unittest.TestCase):
    def setUp(self):
        ...
        with session.make_session_for_existing_admin() as admin_session:
            ...
            admin_session.assert_icommand('''imeta set -R ufs1g2 irods::storage_tiering::query "SELECT DATA_NAME, COLL_NAME, USER_NAME, USER_ZONE, DATA_REPL_NUM where RESC_NAME = 'ufs1g2' and DATA_ACCESS_TIME < 'TIME_CHECK_STRING' and DATA_MODIFY_TIME < 'TIME_CHECK_STRING'"''')
            admin_session.assert_icommand('imeta add -R rnd0 irods::storage_tiering::minimum_delay_time_in_seconds 1')
            admin_session.assert_icommand('imeta add -R rnd0 irods::storage_tiering::maximum_delay_time_in_seconds 2')
            admin_session.assert_icommand('imeta add -R rnd1 irods::storage_tiering::minimum_delay_time_in_seconds 1')
            admin_session.assert_icommand('imeta add -R rnd1 irods::storage_tiering::maximum_delay_time_in_seconds 2')
...
class TestStorageTieringPluginCustomMetadata(ResourceBase, unittest.TestCase):
    def setUp(self):
        ...
        with session.make_session_for_existing_admin() as admin_session:
            ...
            admin_session.assert_icommand('''imeta set -R rnd1 irods::custom_storage_tiering::query "SELECT DATA_NAME, COLL_NAME, USER_NAME, USER_ZONE, DATA_REPL_NUM where RESC_NAME = 'ufs2' || = 'ufs3' and DATA_ACCESS_TIME < 'TIME_CHECK_STRING' and DATA_MODIFY_TIME < 'TIME_CHECK_STRING'"''')
            admin_session.assert_icommand('imeta add -R rnd0 irods::storage_tiering::minimum_delay_time_in_seconds 1')
            admin_session.assert_icommand('imeta add -R rnd0 irods::storage_tiering::maximum_delay_time_in_seconds 2')
            admin_session.assert_icommand('imeta add -R rnd1 irods::storage_tiering::minimum_delay_time_in_seconds 1')
            admin_session.assert_icommand('imeta add -R rnd1 irods::storage_tiering::maximum_delay_time_in_seconds 2')
...
class TestStorageTieringMultipleQueries(ResourceBase, unittest.TestCase):
    def setUp(self):
        ...
        with session.make_session_for_existing_admin() as admin_session:
            ...
            admin_session.assert_icommand('''imeta add -R ufs0 irods::storage_tiering::query "SELECT DATA_NAME, COLL_NAME, USER_NAME, USER_ZONE, DATA_REPL_NUM where RESC_NAME = 'ufs0' and DATA_ACCESS_TIME < 'TIME_CHECK_STRING' and DATA_MODIFY_TIME < 'TIME_CHECK_STRING'"''')
        ...
        with storage_tiering_configured_with_log():
            with session.make_session_for_existing_admin() as admin_session:
                ...
                custom_violating_query = '''"SELECT {} where RESC_NAME = '{}' ''' \
                                         '''and DATA_ACCESS_TIME < 'TIME_CHECK_STRING ''' \
                                         '''and DATA_MODIFY_TIME < 'TIME_CHECK_STRING'"'''.format(
